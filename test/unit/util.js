...
function getAdmin(client, t, userOrg) {
        var keyPath = path.join(__dirname, util.format('/var/fabric-deploy/fabric-samples/first-network/crypto-config/peerOrganizations/%s.example.com/users/Admin@%s.example.com/msp/keystore', userOrg, userOrg));
        var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
        var certPath = path.join(__dirname, util.format('/var/fabric-deploy/fabric-samples/first-network/crypto-config/peerOrganizations/%s.example.com/users/Admin@%s.example.com/msp/signcerts', userOrg, userOrg));
...
function getOrdererAdmin(client, t) {
        var keyPath = path.join(__dirname, '/var/fabric-deploy/fabric-samples/first-network/crypto-config/ordererOrganizations/example.com/users/Admin@example.com/msp/keystore');
        var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
        var certPath = path.join(__dirname, '/var/fabric-deploy/fabric-samples/first-network/crypto-config/ordererOrganizations/example.com/users/Admin@example.com/msp/signcerts');
        var certPEM = readAllFiles(certPath)[0];
